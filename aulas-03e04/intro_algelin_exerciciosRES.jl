...
# ╔═╡ 19ee9b82-aba1-4f17-9279-5cd417e3d4d9
begin
	using LinearAlgebra
...
# ╔═╡ 9dcc5b76-8b74-4816-9de5-25fd73dcf477
using LsqFit

# ╔═╡ 9dd2be3c-e1a1-4866-b34b-53bf4177ffdc
begin
	using DifferentialEquations
	using DSP
end

# ╔═╡ f28c56f8-2686-4ee5-a0cd-37450bb01e24
using PlutoUI

# ╔═╡ 24884d98-2c8c-11ed-1040-a187b9647bd4
md"""
## Exercício 1

Vamos considerar a realização de um experimento que obedece um modelo polinomial de segundo grau, do tipo $y = a_0 + a_1x + a_2x^2$. São medidos os valores de $x$ e de $y$. Encontre os coeficientes do polinômio resolvendo o sistema $X^TX a = X^Ty$ (projeção) e pela aplicação dos pacotes **LsqFit.jl** e **CurveFit.jl**.
"""

# ╔═╡ 462dac7e-2782-490b-8a4f-bb24f4c0da25
# Lendo e plotando os dados
begin
	d = DataFrame(CSV.File("algelin_ex1.csv"))
	scatter(d.x, d.y, xlabel="x", ylabel="y", label=false)
end

# ╔═╡ 251325d5-b6e1-45bf-bf93-e0a073d28e02
md"""
**RESOLUÇÃO**

Primeiramente, vamos aplicar diretamente a projeção para encontrar os melhor ajuste possível.

"""

# ╔═╡ a1326e96-fbe6-4650-9d0e-66741b64bd4a
length(d.x)

# ╔═╡ 9e4995e2-7e35-4c41-a91b-2e54ec96d84e
X = [fill(1,41) d.x d.x .^2]

# ╔═╡ fd1c27d6-d4af-4e8a-a58f-0ae39cca26f9
a = X' * X \ X' * d.y

# ╔═╡ 2ef4abe1-13d4-4c13-b9f3-9a4cbec53374
begin
	scatter(d.x, d.y, xlabel="x", ylabel="y", label=false)
	xx = -1:0.1:20
	function fitfun(x, a)
	...
	plot!(xx, fitfun.(xx, Ref(a)), label="ajuste", linewidth=2)
end

# ╔═╡ 95d20e3f-840b-47a0-9bb6-d9dee82c1f33
md"**Utilizando o pacote LsqFit**"

# ╔═╡ 574f79d9-10ce-47fd-8186-4442657f9e81
@. model2(x, a2) = a2[1] + a2[2]*x + a2[3]*x^2

# ╔═╡ e1223a65-0c89-4f34-9b55-1e88dcc550ae
# Chute inicial
a2_0 = [1.0,1.0,1.0]

# ╔═╡ 76cf3049-bbe3-49ea-a028-3dff179a660d
a2 = curve_fit(model2, d.x, d.y, a2_0)

# ╔═╡ 2d684fd5-7b21-4016-a86e-7c04cbb0801f
begin
	scatter(d.x, d.y, xlabel="x", ylabel="y", label=false)
	plot!(xx, fitfun.(xx, Ref(a2.param)), label="ajuste", linewidth=2)
end

# ╔═╡ cd7b2f2e-90ca-495e-8d5c-6c1b0174b8ae
md"**Usando pacote CurveFit**"

# ╔═╡ 0c52e709-b7d9-40d5-ae42-a89816098265
import CurveFit

# ╔═╡ 366c8018-0eab-4efa-9800-42f79fe612b3
a3 = CurveFit.poly_fit(d.x, d.y, 2)

# ╔═╡ 7a4685f1-cc0a-4a0e-9ff9-094146fcedcb
begin
	scatter(d.x, d.y, xlabel="x", ylabel="y", label=false)
	plot!(xx, fitfun.(xx, Ref(a3)), label="ajuste", linewidth=2)
end

# ╔═╡ 3b3e8005-dec6-44c3-b6a0-23844d550a91
md"**Comparando os três ajustes obtidos**"

# ╔═╡ eb8342b0-292e-440f-92a3-e85abae999a3
hcat(a, a2.param, a3)

# ╔═╡ 94702b26-44a0-4e3b-bae9-3642a7dffd43
begin
	scatter(d.x, d.y, xlabel="x", ylabel="y", label=false)
	plot!(xx, fitfun.(xx, Ref(a)), label="ajuste 1", linewidth=2)
	plot!(xx, fitfun.(xx, Ref(a2.param)), label="ajuste 2", linewidth=3, line=:dash)
	plot!(xx, fitfun.(xx, Ref(a3)), label="ajuste 3", linewidth=5, line=:dot)
end

# ╔═╡ 9ae826d1-bfc1-4b9e-96fa-dc9c638fb514
md"""
## Exercício 2

Dado o sistema massa-mola de dois graus de liberdade abaixo, com amortecimento desprezível, assumindo que m$_1$=m$_2$ = 1 kg e que k = 100 N/m, encontre e interprete as frequências naturais e modos de vibrar do sistema.

![](https://www.researchgate.net/profile/Marco-Faria/publication/294889796/figure/fig2/AS:388500139593740@1469637059848/Figura-5-Sistema-massa-mola-de-dois-graus-de-liberdade.png)

**Dica:** Faça o diagrama de corpo livre de cada massa, encontre o sistema de equações diferenciais que descrevem o sistema, escreva no formato matricial e chegue no problema de autovalor e autovetor admitindo que a resposta x é do tipo $\phi$cos($\omega$t).
"""

# ╔═╡ dbd8eace-d612-47e3-8603-bea3661082d6
md"""
**RESOLUÇÃO**

Fazendo o diagrama de corpo livre na massa 1, encontramos:

$$m_1\ddot{x}_1 = k(x_2 - x_1) - k(x_1)$$
$$m_1\ddot{x}_1 + 2kx_1 -kx_2 = 0$$

Sendo $\ddot{x}_1$ a aceleração da massa 1. Já para a massa 2, 

$$m_2\ddot{x}_2 = -k(x_2 - x_1) - k(x_2)$$
$$m_2\ddot{x}_2 -kx_1 + 2kx_2 = 0$$ 

Sendo assim, o sistema final de equações diferenciais pode ser escrito da seguinte forma:
...
\rightarrow \mathbf{M\ddot{x}} + \mathbf{Kx} = \mathbf{0}$$

Admitindo solução do tipo $x = \phi cos(\omega t)$, temos que $\ddot{x} = -\omega^2 \phi cos(\omega t)$. Substituindo na equação acima, encontramos, para cada par $\omega, \phi$:

$$(\mathbf{K} - \omega^2 \mathbf{M})\phi cos(\omega t) = 0$$

Como $cos(\omega t)$ não vale zero, $(\mathbf{K} - \omega^2 \mathbf{M})\phi = 0$. 

Note que neste exercício $\mathbf{M} = \mathbf{I}$. Sendo assim, o que temos é o problema de autovalor e autovetor em seu formato tradicional:

$$(\mathbf{K} - \omega^2 \mathbf{I})\phi  = 0$$

Os autovalores da matriz $\mathbf{K}$ serão os quandrados das frequências naturais. Já os autovetores serão os modos de vibrar do sistema.

**OBS.:** Caso a matriz de massa não fosse igual à identidade, bastava multiplicar os dois lados da equação pelo inverso da matriz de massa. Como essa matriz é diagonal (o acoplamento das equações do movimento se dá apenas na rigidez), ela admite inversa (fácil de calcular).
"""

# ╔═╡ 21e3e607-af31-46b8-bea8-91184de5e006
# Matrix K para o problema proposto (k = 100 N/m)
K = [[200, -100] [-100, 200]]

# ╔═╡ 37a23a67-7ff1-4ce6-b534-2401cdb3e2f6
ω = sqrt.(eigvals(K))

# ╔═╡ 81d9b404-fd18-40b7-ba9c-28199d82ea24
ϕ = eigvecs(K)

# ╔═╡ 6fdbabb5-3674-441f-8e6a-72e33e20818e
md"""
O sistema apresenta, como resultado, duas frequências naturais. A mais baixa, de 10 rad/s, corresponde ao caso em que as duas massas transladam sempre no mesmo sentido, o que pode ser verificado pelo modo de vibrar correspondente (-0,707, -0,707). 

Já a frequência natural mas alta, de aproximadamente 17,32 rad/s, corresponde ao caso em que as massas se deslocam em sentidos opostos (vide segundo modo de vibrar).

Apenas como verificação, vamos construir o modelo utilizando a biblioteca DifferentialEquations.jl e vamos encontrar a resposta e as frequências naturais do sistema. 
"""

# ╔═╡ d742d2be-903d-462d-b369-24b8cca2fb84
md"""
Para criar o modelo, vamos escrever as equações do movimento como um sistema de equações de primeira ordem. Sabendo que

$$\dot{u} = \begin{bmatrix} \ddot{x}_1 \\ \ddot{x}_2 \\ \dot{x}_1 \\ \dot{x}_2  \end{bmatrix} \quad e \quad
u = \begin{bmatrix} \dot{x}_1 \\ \dot{x}_2 \\ x_1 \\ x_2  \end{bmatrix},$$

pode-se encontrar o sistema

$$\dot{u} = \begin{bmatrix} 0 & 0 & -2k & k \\ 0 & 0 & k & -2k \\ 1 & 0 & 0 & 0 \\ 0 & 1 & 0 & 0 \end{bmatrix} u$$

Note que o sistema, para o caso com amortecimento, pode ser escrito da seguinte forma:

$$\dot{u} = \begin{bmatrix} -\mathbf{M^{-1} C} & -\mathbf{M^{-1} K} \\ \mathbf{I} & \mathbf{0}  \end{bmatrix} u$$


"""

# ╔═╡ 66e4808d-2644-45cc-a343-e09e0ab70cb0
# Criando o modelo
function massamola!(du, u, k, t)
	du[1] = -2*k*u[3] + k*u[4]
	du[2] = k*u[3] - 2*k*u[4]
	du[3] = u[1]
	du[4] = u[2]
...
# ╔═╡ b7a28b78-2705-4128-971e-594d6925f532
md"""
Como condições iniciais vamos considerar apenas o deslocamento da massa 2 em uma unidade, ou seja, $u_0 = [0,0,0,1]$.
"""

# ╔═╡ cefd00bc-edfc-4a72-b20c-5ad6b02c7713
begin
	u0 = [0,0,0,1]
	tspan = (0.0,5.0)
	k = 100
	prob = ODEProblem(massamola!,u0,tspan,k)
end

# ╔═╡ 4f1decda-db59-42f7-aaa3-a8259b18d896
# Resolvendo o sistema
sol = solve(prob, saveat=0.01)

# ╔═╡ e6f9e143-434c-4e92-a141-d51c16a87cb7
begin
	plot(sol, idxs=(0,3), label="x1")
	plot!(sol, idxs=(0,4), label="x2")
end

# ╔═╡ 12d65536-eaf4-496a-ac64-524eae8f666c
md"""
É possível perceber que a resposta de cada uma das massas corresponde à soma de senoidais. Vamos calcular o periodograma do sinal da massa 1.
"""

# ╔═╡ cb800536-f883-41f0-9ec6-267a61efd85e
spec1 = periodogram(sol[3,:], fs=100)

# ╔═╡ e79acc6c-ea2a-402a-953e-2ba5bf994a20
begin
	plot(2*π*spec1.freq[1:40], spec1.power[1:40], label=false, xlabel="Freq (rad/s)")
	plot!([ω[1],ω[1]],[0,maximum(spec1.power)], c=:red, line=:dash, label=false)
	plot!([ω[2],ω[2]],[0,maximum(spec1.power)], c=:red, line=:dash, label=false)
end

# ╔═╡ e48c628a-f400-40a4-9961-3a7abef3ce93
md"""
Perceba que as frequências são as mesmas que haviam sido encontradas anteriormente. Altere a condição inicial e veja o comportamento do sistema.

* Se $u_0 = [0,0,1,1]$, apenas a frequência mais baixa é identificada e o primeiro modo de vibrar pode ser visto na resposta, com as duas massas se deslocando no mesmo sentido sempre.

* Se $u_0 = [0,0,-1,1]$, apenas a frequência mais alta é identificada e o segundo modo de vibrar pode ser visto na resposta, com as duas massas se deslocando em sentidos contrários.

"""

# ╔═╡ 1945902d-9b38-4cda-8e4e-776146da20eb
md"""
## Exercício 3

Faça a decomposição SVD da imagem dada abaixo e explore a reconstrução utilizando parte dos valores singulares.
"""

# ╔═╡ 26e8260f-23d1-484c-80d9-613222a407b1
im = float.(load("fig_algelin_ex3.jpg"))

# ╔═╡ 680370b3-99af-40de-8880-a732b430d9f8
md"""
**RESOLUÇÃO**

Pode-se trabalhar com apenas um canal da imagem, ou seja, com a imagem em tons de cinza. Neste caso, contudo, iremos aplicar a decomposição SVD em cada um dos 3 canais e utilizar apenas os primeiros valores singulares e vetores singulares, que explicam as maiores variâncias, na reconstrução.

O médodo aplicado pode ser visto em detalhes na documentação de JuliaImages
[https://juliaimages.org/v0.22/democards/examples/color_channels/color_separations_svd/](https://juliaimages.org/v0.22/democards/examples/color_channels/color_separations_svd/)
"""

# ╔═╡ b5f5777a-1d2a-4728-a7fd-d153c433d55c
ch = channelview(im);

# ╔═╡ 43fb24c3-88d3-41c4-beeb-aabf0e695bc4
size(ch) # Note que os 3 canais (RGB) foram separados em um array

# ╔═╡ ba45b631-a925-4b11-a020-ade4c0335632
md"""
Com os canais separados, vamos fazer a decomposição SVD para cada um deles.
"""

# ╔═╡ e0648f82-e9df-4538-95bf-db2a3361f946
svdfac = svd.(eachslice(ch; dims=1));

# ╔═╡ 48985086-704e-440c-800b-a4efa2dd10ed
typeof(svdfac)

# ╔═╡ 5dd9bf17-8e5e-4b02-b067-ee1d7b6c77d7
md"""
**Reconstrução**
"""

# ╔═╡ 40225918-d416-4ddc-8b70-a9f55d6f48c0
# Reconstrução considerando rank r
...
# ╔═╡ 88207e8c-0d64-4352-8866-daa918b08316
@bind r Slider(1:100)

# ╔═╡ 837be2ba-4538-4232-b50d-5fd584a462ee
r

# ╔═╡ 61f9d83e-2484-4bd0-860d-077733f0e800
colorview(RGB, rank_approx.(svdfac, r)...)

# ╔═╡ 35675156-7531-4f24-bb2a-e5d213be5a58
md"**Explore o valor de r com o Slider.**"
