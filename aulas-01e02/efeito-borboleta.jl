...
# ╔═╡ e3bcf9bc-880d-4ccf-9f4e-f5f11723a5fb
...
# ╔═╡ b4d52400-1ce1-11ed-38ed-bba8914879be
md"""
# Efeito borboleta

Vamos brincar um pouco com sistemas caóticos.

## Equações de Lorenz

Talvez o sistema dinâmico caótico mais conhecido. 

$\frac{dx}{dt} = σ\cdot(y-x)$

$\frac{dy}{dt} = x\cdot(ρ - z) - y$

$\frac{dz}{dt} = x\cdot y - β\cdot z$

Nestas equações, σ, ρ e β são parâmetros externos que serão variados.

## Solução numérica com Julia
"""

# ╔═╡ 5963bba6-9a01-4baf-b6d9-5d926628770e
...
# ╔═╡ ca0173f0-d14b-4c80-8146-98852ee9bb1b
md"""
Intervalo de integração: $(@bind tmax Slider(1.0:1.0:1000, default=100.0))

x₀ = $(@bind x0 Slider(1.0:0.00001:1.01))
y₀ = $(@bind y0 Slider(0.0:0.00001:0.01))
z₀ = $(@bind z0 Slider(0.0:0.00001:0.01))

σ = $(@bind σ Slider(0.0:0.01:50.0, default=10.0))
ρ = $(@bind ρ Slider(0.0:0.02:100.0, default=28.0))
β = $(@bind β Slider(0.0:0.001:10.0, default=8/3))
"""

# ╔═╡ f350985d-832a-4da0-94bf-71b53aa364df
sol = let
	u0 = [x0, y0, z0] # Valores iniciais
	p = (σ=σ, ρ=ρ, β=β) # Parâmetros
	tspan = (0.0, tmax) # Intervalor de integração
	prob = ODEProblem(lorenz!, u0, tspan, p)
	solve(prob)
end

# ╔═╡ 6221a6f1-3259-49a1-89fa-996bd95a59d1
plot(sol, idxs=(1,2,3))

# ╔═╡ 2f45cce3-2152-497b-a5d4-ef0fdd538d90
md"""
tmax = $tmax, x₀ = $x0, y₀= $y0, z₀ = $z0

σ = $σ, ρ = $ρ, β = $β
"""

# ╔═╡ 527474bf-65d0-47c9-b871-2e95f2e52b59
plot(sol)
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "6533e33b9f3f8ebc51f1bcac7080778b30a1ecad"
